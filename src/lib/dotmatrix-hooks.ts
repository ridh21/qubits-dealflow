...
export function useCyclePhase({ active, cycleMsBase, speed = 1 }: UseCyclePhaseOptions): number {
  ...
  useEffect(() => {
    // The resting phase is derived on the way out instead of being written
    // here: a synchronous setState in an effect body cascades an extra render.
    if (!active) return;
...
export function useSteppedCycle({
  ...
  useEffect(() => {
    if (!active) {
      ...
      // -1 matches no real step, so the next activation always publishes its
      // first frame. Writing state synchronously here would cascade a render,
      // and the hook already reports idleStep while inactive.
