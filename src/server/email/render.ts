...
export function renderEmail(opts: { title: string; intro: string; bodyLines?: string[]; cta?: { label: string; href: string } }) {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f7fafc;font-family:Inter,Arial,sans-serif;color:#18181b">
  <table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:32px 16px">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #ececec;border-radius:12px">
      <tr><td style="padding:24px 28px;border-bottom:1px solid #f1f1f1;font-weight:700;font-size:15px;color:#357dff">DealFlow360</td></tr>
      <tr><td style="padding:28px">
        ...
        ${opts.cta ? `<p style="margin:24px 0 0"><a href="${opts.cta.href}" style="background:#357dff;color:#fff;text-decoration:none;padding:10px 18px;border-radius:24px;font-size:14px;display:inline-block">${escapeHtml(opts.cta.label)}</a></p>` : ""}
      </td></tr>
    </table>
