...
### Report pagination and recovery

- `14720cd`: staff and portal route errors share a private recovery screen with a Phosphor retry action in the sidebar. Unexpected report exceptions reach that boundary instead of exposing database messages; validation errors remain actionable.
- `5e611d7`: each report table stores page/size in URL parameters, clamps invalid/out-of-range pages, supports 10/25/50/100 rows, and exposes pagination through sidebar controls. Native history updates preserve the generated report. Browser verification confirmed quotation page 2 persists on refresh, while export links retain the original report filters. Report axe scan: zero violations; one offscreen sidebar contrast check remains incomplete.
- The public demo currently exposes a data invariant failure on `INV-09509`: its cash payment already covers the invoice and a further credit is applied. The independently edited demo seed applies `sd_cn1` after a FULL payment on `i09`. No public data was changed during this audit. Demo seed verification must catch and repair this before phase 13 completion.
- Existing database tests also assume stable `@dealflow360.test` users and Acme fixtures; the independently changed demo seed now uses other identities. Separate deterministic integration fixtures from the evolving demo seed before claiming clean-database CI coverage.
- Quotation builder/list and billing-job observability work is in progress separately; repeat the full validation after those changes land. The last successful production build predates these in-progress changes.

### Consolidated PR checkpoint

- Reviewed merged GitHub PRs #1 and #3–#6 and reconciled current `origin/main` without discarding the separately edited local README. The only business-code merge conflict retained the newer exact recurring-price basis.
- Seed commit `e607b12` now owns the INR/GST demo scenarios and fixes the earlier `INV-09509` overapplication. Read-only verification of the configured database found 27 quotations, 8 orders, 8 subscriptions, 21 invoices and 4 credits. All invoice line totals, payment totals and credit application totals reconcile; no invoice is overpaid. The previous seed-data failure recorded above is resolved.
- Core feature implementations exist for phases 03–11. Phase 12 UI acceptance and phase 13 end-to-end/operational hardening remain active; unfinished acceptance criteria in earlier phases still prevent full sign-off. Counting strict completed phases conservatively: 3 user-completed baseline phases, 9 core-implemented phases awaiting final acceptance, 2 phases still under implementation.
- Latest consolidated default suite: 315 passed; 44 database tests skipped. Typecheck and production build passed. The suite's database skips are not evidence of end-to-end completion. Stable clean-database fixtures are being prepared separately from the editable demo seed.
- Quotation customer switching, per-revision dismissal persistence, snapshot diffs, complete list filters/board, and durable billing run history/retry have landed. Billing jobs and create-quotation actions are available through the shadcn sidebar with Phosphor icons.
